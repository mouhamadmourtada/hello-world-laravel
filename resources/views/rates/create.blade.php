    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center">
                <h1 class="text-2xl font-semibold text-gray-900">Nouveau Tarif</h1>
            ...
            <div class="mt-6">
                <form action="{{ route('rates.store') }}" method="POST" class="space-y-8">
                    @csrf
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select id="duration_type" name="duration_type" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                    ...
                                    <option value="night" {{ old('duration_type') == 'night' ? 'selected' : '' }}>Par nuit</option>
                                    <option value="3_days" {{ old('duration_type') == '3_days' ? 'selected' : '' }}>3 jours</option>
                                    <option value="week" {{ old('duration_type') == 'week' ? 'selected' : '' }}>Par semaine</option>
                                    <option value="month" {{ old('duration_type') == 'month' ? 'selected' : '' }}>Par mois</option>
                                    <option value="year" {{ old('duration_type') == 'year' ? 'selected' : '' }}>Par an</option>
                                </select>
                                @error('duration_type')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <input type="number" step="0.01" min="0" name="price" id="price" value="{{ old('price') }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pr-12 sm:text-sm border-gray-300 rounded-md">
